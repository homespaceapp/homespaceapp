...
const ROLES = ['Hydraulik', 'Elektryk', 'Zarządca', 'Dozorca', 'Lekarz', 'Dentysta', 'Weterynarz', 'Apteka', 'Rodzina', 'Sąsiad', 'Inne'];
...
export default function KontaktyClient({ initialContacts }: { initialContacts: Contact[] }) {
  ...
  const [editForm, setEditForm] = useState({ name: '', role: '', phone: '', email: '', notes: '' });
  ...
  const ContactForm = ({ values, onChange, onSubmit, onCancel, showDelete, onDelete }: {
    values: typeof form; onChange: (v: typeof form) => void;
    ...
    <form onSubmit={onSubmit} className="bg-white rounded-xl p-5 border border-zinc-200 shadow-sm">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
        <input value={values.name} onChange={e => onChange({ ...values, name: e.target.value })}
          placeholder="Imię / Nazwa *" required
          className="px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-emerald-500"
        />
        <select value={values.role} onChange={e => onChange({ ...values, role: e.target.value })}
          className="px-3 py-2 text-sm border border-zinc-200 rounded-lg bg-white"
        >
          <option value="">— Rola —</option>
          {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
        </select>
        <input value={values.phone} onChange={e => onChange({ ...values, phone: e.target.value })}
          placeholder="Telefon" type="tel"
          className="px-3 py-2 text-sm border border-zinc-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-emerald-500"
        />
        <input value={values.email} onChange={e => onChange({ ...values, email: e.target.value })}
      ...
      <div className="flex gap-2">
        ...
        {showDelete && <button type="button" onClick={onDelete} className="ml-auto px-4 py-2 text-sm text-red-500 hover:text-red-700">Usuń</button>}
      </div>
    </form>
  );
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-0.5">Hydraulik, lekarz, zarządca…</p>
        </div>
      ...
      {showForm && (
        <div className="mb-5">
          <ContactForm values={form} onChange={setForm} onSubmit={handleAdd} onCancel={() => setShowForm(false)} />
        </div>
      )}
      ...
      {contacts.length === 0 ? (
        <div className="bg-white rounded-xl p-8 border border-zinc-200 text-center">
          ...
          <p className="text-sm text-zinc-400 mt-1">Dodaj hydraulika, lekarza, zarządcę…</p>
        </div>
      ) : (
            ...
            <div key={c.id}>
              {editId === c.id ? (
                <ContactForm
                  ...
                  showDelete
                  onDelete={() => { setEditId(null); handleDelete(c.id); }}
                />
              ) : (
                <div
